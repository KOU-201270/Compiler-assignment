...
public class EqExpTranslator {
    ...
    public static Value translate(Node node,
        ...
        if (node.getSons().size() == 1) {
            return RelExpTranslator.translate(node.getSons().get(0),
                    intermediateCodes, symbolTable);
        }
        Value v1 = EqExpTranslator.translate(node.getSons().get(0),
                intermediateCodes, symbolTable);
        Value v2 = RelExpTranslator.translate(node.getSons().get(2),
                intermediateCodes, symbolTable);
        ...
        if (v1.getType() == ValueType.NUMBER && v2.getType() == ValueType.NUMBER) {
            ...
            switch (token.getType()) {
                case EQL:
                    intermediateCodes.add(new Quadraple(tempVal, v1, v2, CalcType.EQL));
                    break;
                default:
                    intermediateCodes.add(new Quadraple(tempVal, v1, v2, CalcType.NEQ));
                    break;
            }
            return tempVal;
        }
